...
extension Session {
    ...
    func getUserServices(id: String, page: UInt, rows: UInt) -> Observable<[Service]> {
        
        return response(APIRequest.getUserServices(key: "", id: id, page: page, rows: rows)).map { response in
